...
var wid = 100; //it has to be square and it has to be EVEN
...
function match_score(col, row) {
 ...
 function sym_score(col, row) {
    ...
    opposite_row = opposite_index(row);
    opposite_col = opposite_index(col);
    var col_to_compare = col_array[col][row];
...
function set_color(id, cc) {
    for (var i=0; i<wid; i++) {
        for (var j=0; j<wid; j++) {
            //alert('changing ' + i + ' ' + j + ' ' + col_array[i][j]);
            col_index = col_array[i][j];
...
function update_sim(analyser, frequencyData, id, cc) {
    ...
    requestAnimationFrame(function() { //using extra function to pass in arggs successfully
        update_sim(analyser, frequencyData, id, cc);
    });
    */
