...
//FROM THIS THING (CWILSO) https://github.com/cwilso/PitchDetect/blob/master/js/pitchdetect.js
...
function title_screen(analyser, frequencyData, cc, audio_level, ac) {
    cc.drawImage(back_image, 0, 0);
    if ((super_time_counter % 2) == 0) {
        update_flyer(cc, audio_level, ac);
    }
    
    draw_the_flyer(cc, ac);
...
function update_flyer(cc, audio_level, ac) {
    if (ac > 0) {
        //alert(ac_to_position(ac));
        fly_vel = Math.round((ac_to_position(ac) - fly_pos) * (audio_level/(weight*300)));
    }
...
function ac_to_position(ac) {
    return 12000 - Math.log(ac)*2000;
}

function run_game(analyser, frequencyData, cc, context) {
    ...
    if ((super_time_counter % 2) == 0) {
        ...
        document.getElementById("titlerator").innerHTML = ac_to_position(ac);

        show_accel(audio_level);
    }
    
    if (true) {
        title_screen(analyser, frequencyData, cc, audio_level, ac);
    } else if (crashed) {
        run_crash(cc);
    }else {
...
function main_func() {
    ...
    navigator.getUserMedia({audio: true}, function(stream) {
        ...
        intervalhandle = setInterval(function(){run_game(analyser, frequencyData, cc, context)}, 40);
